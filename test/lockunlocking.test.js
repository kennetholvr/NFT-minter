...
contract("AisthisiToken: test mint and lock", (accounts) => {
  ...
  it("is possible to mint tokens for the minter role", async () => {
    ...
    await truffleAssert.fails(
      token.transferFrom(deployerAddress, tokenHolderOneAddress, 0)
    ); //transferring for others doesn't work

    //but transferring in general works
    await truffleAssert.passes(
      token.transferFrom(tokenHolderOneAddress, tokenHolderTwoAddress, 0, {
        from: tokenHolderOneAddress,
      })
    );
  });
  ...
  it("is not possible to transfer locked tokens", async () => {
    ...
    await truffleAssert.fails(
      token.transferFrom(tokenHolderTwoAddress, tokenHolderOneAddress, 0, {
  ...
  it("is possible to unlock the token and transfer it again", async () => {
    ...
    await truffleAssert.passes(
      token.transferFrom(tokenHolderTwoAddress, deployerAddress, 0, {
        from: tokenHolderTwoAddress,
      })
    );
    ...
    assert.equal(
      tokenOwner,
      deployerAddress,
      "The Owner is not the correct address"
    );
  });
